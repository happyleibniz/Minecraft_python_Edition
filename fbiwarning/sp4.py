...
class Sphere:
    def __init__(self, radius, slices, stacks, disk_radius, disk_thickness):
        self.disk_texture = pyglet.image.load("space.jpg").get_texture()
        self.radius = radius
        self.slices = slices
        self.stacks = stacks
    ...
    def create_disk_vertices(self):
        ...
        for i in range(num_segments + 1):
            ...
            tex_coords.extend([i / num_segments, 1.0])  # Set texture coordinates
        ...
        for i in range(num_segments + 1):
            ...
            vertices.extend([x, -self.disk_thickness / 2.0, z])
            normals.extend([0.0, -1.0, 0.0])
            tex_coords.extend([i / num_segments, 0.0])  # Set texture coordinates
        ...
        for i in range(num_segments):
            ...
            indices.extend([top_curr, bottom_curr, bottom_next])
            indices.extend([top_curr, bottom_next, top_next])
    ...
    def draw(self):
        ...
        glBindTexture(GL_TEXTURE_2D, self.disk_texture.id)  # Bind disk texture
...
class Window(pyglet.window.Window):
    def __init__(self, width, height, title='Pyglet Sphere'):
        ...
        glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, (GLfloat * 4)(0.0, 0.0, 0.0, 1.0))
